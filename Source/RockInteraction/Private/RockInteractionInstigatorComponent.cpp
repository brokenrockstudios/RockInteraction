...
#include "RockInteractionInstigatorComponent.h"
...
URockInteractionInstigatorComponent::URockInteractionInstigatorComponent(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
void URockInteractionInstigatorComponent::BeginPlay()
{
	Super::BeginPlay();
...
void URockInteractionInstigatorComponent::OnControllerChanged(APawn* Pawn, AController* OldController, AController* NewController)
{
	const bool bShouldRun = Pawn->IsLocallyControlled() || Pawn->HasAuthority();
...
void URockInteractionInstigatorComponent::AddPersistentCandidate(const TScriptInterface<IRockInteractableTarget>& Target)
{
	if (Target)
	{
...
void URockInteractionInstigatorComponent::RemovePersistentCandidate(const TScriptInterface<IRockInteractableTarget>& Target)
{
	PersistentCandidates.RemoveSwap(Target);
}

void URockInteractionInstigatorComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void URockInteractionInstigatorComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void URockInteractionInstigatorComponent::StartSphereScan()
{
...
void URockInteractionInstigatorComponent::TickSphereScan()
{
...
void URockInteractionInstigatorComponent::UpdateCandidates(TArray<FOverlapResult>& Overlaps)
{
...
void URockInteractionInstigatorComponent::OnCandidateEntered(const TScriptInterface<IRockInteractableTarget>& Target)
{
	// Do game specific things, such as adding GAS abilities based upon candidates.
}

void URockInteractionInstigatorComponent::OnCandidateExited(const TScriptInterface<IRockInteractableTarget>& Target)
{
	// Do game specific things, such as adding GAS abilities based upon candidates.
}

void URockInteractionInstigatorComponent::OnCandidatesUpdated(const TArray<TScriptInterface<IRockInteractableTarget>>& NewCandidates)
{
	// Do game specific things, such as adding GAS abilities based upon candidates.
}
...
void URockInteractionInstigatorComponent::ScoreAndSelectFocused()
{
...
void URockInteractionInstigatorComponent::DrawInteractionPointDebug(const UWorld* World, const FVector& Location, float LookAtDotProduct)
{
#if ENABLE_DRAW_DEBUG
	const bool bShowPoints = CVarShowInteractionPoints.GetValueOnGameThread();
	if (!bShowPoints)
...
bool URockInteractionInstigatorComponent::GetViewPoint(FVector& OutOrigin, FVector& OutDirection) const
{
	const APawn* Pawn = Cast<APawn>(GetOwner());
	if (!Pawn)
	{
...
bool URockInteractionInstigatorComponent::HasFocus() const
{
	return bHasFocus;
}

int32 URockInteractionInstigatorComponent::GetOptionCount() const
{
	return CurrentOptions.AvailableOptions.Num();
}

const FRockInteractionContext& URockInteractionInstigatorComponent::GetFocusedContext() const
{
	return CurrentContext;
}

const FRockInteractionOptions& URockInteractionInstigatorComponent::GetFocusedOptions() const
{
	return CurrentOptions;
}

void URockInteractionInstigatorComponent::SetFocusedTarget(const TScriptInterface<IRockInteractableTarget>& NewTarget)
{
	...
	if (NewTarget)
	{
		...
		{
			NewDelegate->AddUObject(this, &URockInteractionInstigatorComponent::OnFocusedTargetStateChanged);
		}
	}
}

void URockInteractionInstigatorComponent::OnFocusedTargetStateChanged()
{
...
void URockInteractionInstigatorComponent::ClearFocus()
{
	if (!bHasFocus) { return; }
...
void URockInteractionInstigatorComponent::TriggerInteraction(int32 OptionIndex)
{
	if (!bHasFocus)
	{
		return;
